...
```bash
anygen task +run --operation doc --prompt <text> [flags]
```
...
| `--export-format` | — | docx | Export format |
...
Run `anygen task +run --operation doc --prompt '...' --output-dir ./output` in the background.
Tell the user: the document is being generated, and they can continue chatting.

When complete, notify the user with:
- The preview URL
- Thumbnail or preview images (if available)
- Ask if they want to download the file locally (DOCX)
