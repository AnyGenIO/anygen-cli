...
```bash
anygen task +run --operation smart_draw --prompt <text> [flags]
```
...
| `--export-format` | — | drawio | Export format |
| `--output-dir` | — | — | Download output file to local directory |
| `--timeout` | — | 180000 | Polling timeout in milliseconds |
...
Run `anygen task +run --operation smart_draw --prompt '...' --output-dir ./output` in the background.
Tell the user: the diagram is being generated, and they can continue chatting.
