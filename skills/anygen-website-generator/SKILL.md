...
```bash
anygen task +run --operation website --prompt <text> [flags]
```
...
Run `anygen task +run --operation website --prompt '...' --output-dir ./output` in the background.
Tell the user: the website is being generated, and they can continue chatting.
