---
name: anygen-storybook-generator
version: 1.0.0
...
```bash
anygen task +run --operation storybook --prompt <text> [flags]
```
...
Run `anygen task +run --operation storybook --prompt '...' --output-dir ./output` in the background.
Tell the user: the storybook is being generated, and they can continue chatting.
